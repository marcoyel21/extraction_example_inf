...
#para matrices 
fondo<-t(matrix(fondo$V1,ncol = docs, nrow= 5)) %>% as.data.frame() %>% 
  rename(nombre=V1, sociedad_y_razon=V2, tipo=V3, clase= V4,plazo_categ=V5)
ren_bruto<-t(matrix(ren_bruto$V1,ncol = docs, nrow= 5))%>% as.data.frame() %>%
...
ren_neto<-t(matrix(ren_neto$V1,ncol = docs, nrow= 5))%>% as.data.frame() %>%
